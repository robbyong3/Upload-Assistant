...
class Args():
    ...
    def parse(self, args, meta):
        ...
        parser.add_argument('-lq', '--limit-queue', dest='limit_queue', nargs='?', required=False, help="Limit the amount of queue files procesed", type=int, default=0)
        ...
        parser.add_argument('-tpc', '--trackers-pass', dest='trackers_pass', nargs='?', required=False, help="How many trackers need to pass all checks (dupe/banned group/etc) to actually proceed to uploading", type=int)
